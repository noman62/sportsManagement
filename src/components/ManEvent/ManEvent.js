...
const ManEvent = () => {
  ...
  const allIndoor = data.filter(
    item => item.gender === 'Man' && item.gameType === 'Indoor'
  )

  const allOutdoor = data.filter(
    item => item.gender === 'Man' && item.gameType === 'Outdoor'
  )
        ...
        <div class='container dept text-center'>
          <div class='row'>
            <div class='col-sm-12'>
              <h1>Man's Events</h1>
              <p>
              প্রতি বছর Nstu শারীরিক শিক্ষা অধিদপ্তর অনেকগুলি আকর্ষণীয় পুরুষদের ইনডোর এবং আউটডোর ইভেন্টের আয়োজন করে।
